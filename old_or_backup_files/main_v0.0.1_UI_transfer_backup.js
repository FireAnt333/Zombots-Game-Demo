...
- Implement first-draft game state management, starting with a master object "let gs = {};" with properties keeping track of things like active enemies and turrets, collidable objects, etc. (Keep in mind you only have to check for collisions for each MOBILE object, as permanently stationary objects like walls and turrets will never collide with anything except a mobile object, in which case you're already covered by checking from the perspective of the mobile object.)
...
const canvasWidth = 960; // 240 after scale change
const canvasHeight = 640; // 160 after scale change
const upscaleFactor = 4;
const btnBorderOffset = 2*upscaleFactor;

let cameraOffsetX = canvasWidth/upscaleFactor/2; // 120
let cameraOffsetY = ((canvasHeight/upscaleFactor) -20) / 2; // 70
...
const btnTempStartLevel = new Button('Start Levels', [111, 71], 18, 18, false, 'resources/images/ui_240x160/Button_BuildOptions_18x18.png', null, () => {
  loadScene('gameLevel');
  btnActionPlaceholder('startLevel (temp)');
});
...
// Maybe just combine drawLoadoutList() with drawUIArt()
...
function draw() {
  ...
  if (currentScene === 'gameLevel') {
    drawBackgroundArt(levelData.testLevel);
  }
...
function drawBackgroundArt(level) {
  ...
  // Starts the camera focused on the center of a square map
  let gridOffsetX = -(tileMap.length*tileWidth/2);
  let gridOffsetY = -(tileMap[0].length*tileHeight/2);

  for (let i = 0; i < tileMap.length; i++) {
    for (let j = 0; j < tileMap[i].length; j++) {
      if (true) { //draw only if origin + dimensions put any part of tile in display area
        //if ()
        if (tileMap[i][j] === 1) {
          ...
          tileImg = level.tileSet.ground1.img;
          console.log('Something went wrong and triggered Else');
        }

        cu.drawImage(tileImg, (tileWidth*j) + cameraOffsetX + gridOffsetX, (tileHeight*i) + cameraOffsetY + gridOffsetY, tileWidth, tileHeight);
        //cu.drawImage(tileImg, (tileWidth*j) + gridOffsetX, (tileHeight*i) + gridOffsetY, tileWidth, tileHeight);
        //cu.drawImage(tileImg, tileOriginX + (tileWidth*j) + gridOffsetX, tileOriginY + (tileHeight*i) + gridOffsetY, tileWidth, tileHeight);
      }
    }
  }
};
...
function drawTestInfo() {
  // TEMPORARY TESTING INFORMATION: Display click location and current scene on screen
  cm.fillStyle = 'red';
...
function clickOnCanvas(event) {
  if (firstClick) {
    ...
    mouseX = event.clientX - canvasMain.offsetLeft - canvasContainer.offsetLeft + document.scrollingElement.scrollLeft;
    mouseY = event.clientY - canvasMain.offsetTop - canvasContainer.offsetTop + document.scrollingElement.scrollTop;
...
function keyDownHandler(key) {
  ...
  // A = 65,  left = 37
  // D = 68, right = 39
  console.log('KDH activated');
  if (currentScene === 'gameLevel') {
    if (key.keyCode === 87 || key.keyCode === 38) { // W and Up Arrow
      cameraOffsetY += 4;
      draw();
      //console.log('W/up handled');
    }
    if (key.keyCode === 83 || key.keyCode === 40) { // S and Down Arrow
      cameraOffsetY -= 4;
      draw();
      //console.log('S/down handled');
    }
    if (key.keyCode === 65 || key.keyCode === 37) { // A and Left Arrow
      cameraOffsetX += 4;
      draw();
      //console.log('A/left handled');
    }
    if (key.keyCode === 68 || key.keyCode === 39) { // D and Right Arrow
      cameraOffsetX -= 4;
      draw();
      //console.log('D/right handled');
    }
  }
...
import { uiData } from './data/data_ui.js';
import { levelData } from './data/data_levels.js';
//import { chassisData } from './data/data_chassis.js';
import { weaponData } from './data/data_weapons.js';
import { utilityData } from './data/data_utilities.js';
import { enemyData } from './data/data_enemies.js';
